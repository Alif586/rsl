/**
 * OTP WORKER 1 - SMS Panel Monitoring (Multi-User) D GRUP 
 * Server: 139.99.63.204
...
class OtpWorker1 extends EventEmitter {
    ...
    initializeBots() {
        try {
            // Add better configuration for Telegram bots
            const botOptions = {
                polling: false,
                request: {
                    agentOptions: {
            ...
            // Set webhook to false explicitly
    ...
    /**
     * Enhanced retry mechanism for Telegram messages
     */
...
<pre>${sms.message}</pre>`;
        ...
        try {
            const success = await this.sendTelegramWithRetry(
                this.botGroup,
