...
class imss extends EventEmitter {
    constructor() {
        super();
        this.config = null;
        this.botGroup = null;
    ...
    initializeBots() {
        ...
        this.emit('log', '✅ Bots initialized (Worker 2)');
    }
    ...
    initializeDatabase() {
        ...
        this.NumberModel = conn.model('Number', new mongoose.Schema({
            number: String, country: String, flag: String, status: String, assigned_to: Number
        }));
    ...
    extractOtp(text) {
        if (!text) return null;
        const m = text.match(/(\d{3})[\s-](\d{3})/) || text.match(/(?:code|otp|is).*?(\d{4,8})/i) || text.match(/\b\d{4,6}\b/);
        return m ? (m[1] && m[2] ? m[1]+m[2] : m[1] || m[0]) : null;
    }
    ...
    // ফিক্স: user প্যারামিটার গ্রহণ করা হচ্ছে
    async sendToGroup(sms, user) {
        ...
        // ইউজারের Server array থেকে নাম বের করা হচ্ছে
        ...
        // ফিক্স: Server নাম মেসেজে যুক্ত করা হয়েছে
...
✅ Stay With Us.💖`;
    ...
    async performLogin(user) {
        try {
            ...
            await user.client.post(`${this.BASE_URL}/signin`, params.toString(), {
                headers: { "Origin": this.BASE_URL, "Referer": `${this.BASE_URL}/login` },
                maxRedirects: 0,
    ...
    async startUser(user) {
        ...
        if (await this.performLogin(user)) {
            // লগিন সাকসেস লগে সার্ভার নাম দেখানো হচ্ছে
